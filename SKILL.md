...
HTML is the authoritative workfile and visual baseline. PPTX and PDF are downstream adapters. Their implementation must not simplify or downgrade HTML layout, information density, motion, interaction, editability, or visual quality. If a faithful native PPTX mapping is unavailable, block PPTX publication and identify the unsupported component; never redesign the HTML around a PPT limitation. Export adapters load only on demand and operate on an isolated export copy, not the live Scene DOM.
...
- The media editor must always allow image replacement, alt-text editing, `contain`/`cover` selection, 50%–200% in-frame scaling, and horizontal/vertical focal positioning. Diagrams, screenshots, and text-heavy images default to `contain`; use `cover` only for photos or when deliberate cropping is acceptable. These values persist in the same HTML workfile and downstream model. Resizing the media frame itself is a structural layout revision: the Agent edits the Scene source and reruns collision QA; the browser editor must not expose unconstrained frame dragging.
...
- Collaboration package IDs are namespaced on merge; workfile report ID, section completeness, design contract, SHA-256 integrity, and revision lineage are hard gates. File names are not identities. Identical hashes deduplicate; a strict descendant wins; same-revision differences or divergent lineage block. Merge preserves source ledgers and user edits and may not summarize the sections again. ID remapping is structural: base64, data URLs, binary assets, source prose, quotations, and hashes are opaque and must remain byte-for-byte unchanged.
...
- PPTX and PDF are downstream adapters only: they may apply format-specific static fallbacks, but must never feed layout compromises back into Scene HTML. Formal PDF publication uses a Scene Capture Gate followed by a PDF Artifact Gate with rendered-page coverage and perceptual comparison. PPTX publication blocks missing media, unreadable contrast, out-of-bounds objects, severe text auto-shrink, and major visual loss against the same-run HTML Scene captures.
- Necessary information remains complete with motion disabled and in PDF.
- A PDF is not current merely because the file and hash exist. Use `prepareForExport → renderSceneToBitmap → assemblePdf → verifyPdf`; never fall back to `window.print()`. Scene Capture Gate must pass before PDF assembly, then PDF Artifact Gate verifies page count, ratio, coverage, and perceptual consistency. Blank, near-blank, incomplete-media, or stale-dependency output blocks delivery.

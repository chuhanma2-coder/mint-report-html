...
if (command === 'prepare') { run('prepare-creative.mjs', process.argv.slice(3)); process.exit(0); }
...
try {
  ...
  const model = { sceneById: Object.fromEntries(brief.scenes.map(s => [s.id, s])), atoms: Object.fromEntries(map.contentAtoms.map(a => [a.id,a.text])) };
  ...
  const visualArgs = [report,visual,path.join(candidate,'visual-qa'),`--profile=${profile}`, ...(profile === 'revision' ? [`--scenes=${affected.join(',')}`] : [])];
  ...
  if (exportPdf) {
    ...
    run('export-creative-pdf.mjs',[report,path.join(candidate,pdfName),path.join(candidate,exportName),`--kind=${profile === 'publish' ? 'formal' : 'preview'}`]);
  ...
  build.outputs = { ...build.outputs, html: { file: 'report.html', hash: sha(fs.readFileSync(report)), state: 'current' }, formalPdf: profile === 'publish' ? { file: pdfName, hash: sha(fs.readFileSync(path.join(candidate,pdfName))), state: 'current' } : { ...(typeof build.outputs?.formalPdf === 'object' ? build.outputs.formalPdf : {}), state: 'stale' } };
  ...
  const delivery = { schemaVersion: '0.10.0-rc.1', status: current.deliveryStatus, structureState: current.structureState, qaProfile: profile, sceneOrder: current.currentSceneOrder, checks: { staticQa: read(qa).passed === true, browserQa: read(visual).passed === true, ...(profile === 'publish' ? { pdfCurrent: read(path.join(candidate,exportName)).status === 'matched' } : {}) }, outputs: build.outputs };
  ...
  write(path.join(candidate,'performance-report.json'), { phase: profile, elapsedMs: Date.now()-started, checkedScenes: affected, assembly: read(path.join(candidate,'assembly-report.json')), normalizationRuns: 0, modelCallsByScripts: 0, steps });
